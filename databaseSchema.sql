-- Complete Database Migration for New Survey Questions
...
ALTER TABLE survey_responses
  ADD COLUMN IF NOT EXISTS photo_preference VARCHAR(50),
  ADD COLUMN IF NOT EXISTS social_media_preference VARCHAR(50),
  ADD COLUMN IF NOT EXISTS will_pose VARCHAR(50),
...
COMMENT ON COLUMN survey_responses.photo_preference IS 'Posed or action, what''s your favorite?';
COMMENT ON COLUMN survey_responses.social_media_preference IS 'Which would you share on social media?';
COMMENT ON COLUMN survey_responses.will_pose IS 'Will you stop 30 seconds to pose?';
COMMENT ON COLUMN survey_responses.price_preference IS 'What price feels fair for one photo?';
COMMENT ON COLUMN survey_responses.buy_immediately IS 'Would you buy if ready immediately?';
...
-- Insert sample survey responses with NEW survey questions
INSERT INTO survey_responses (
  id, bib_number, selected_photo_ids, runner_name, runner_email, 
  photo_preference, social_media_preference, will_pose, price_preference, buy_immediately,
  completed_at
) VALUES
  -- Bib 1001: Survey completed with diverse preferences
  ('aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaaa', '1001', 
   ...
   'action', 'both', 'yes', '10-14', 'yes',
   NOW() - INTERVAL '2 hours'),
  
  -- Bib 1002: Survey completed with posed preference
  ('bbbbbbbb-bbbb-4bbb-bbbb-bbbbbbbbbbbb', '1002', 
   ...
   'posed', 'posed', 'maybe', '15-19', 'yes',
   NOW() - INTERVAL '3 hours'),
   
  -- Bib 1003: Survey completed with budget-conscious responses
...
BEGIN
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Survey schema updated with new questions:';
    RAISE NOTICE '- photo_preference';
    RAISE NOTICE '- social_media_preference';
    RAISE NOTICE '- will_pose';
    RAISE NOTICE '- price_preference';
    RAISE NOTICE '- buy_immediately';
    RAISE NOTICE '========================================';
